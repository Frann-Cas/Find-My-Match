...
export default function FindPartner() {
  ...
  async function saveProfile() {
    setLoading(true)
    await supabase.from('player_profiles').upsert({user_id:profile.id,...setup},{onConflict:'user_id'})
    setMyProfile({...setup}); setShowSetup(false); setLoading(false)
  }
  ...
  async function toggleAvailable() {
    ...
    navigator.geolocation?.getCurrentPosition(
      async pos=>{
        await supabase.from('player_profiles').upsert({user_id:profile.id,available_to_play:v,latitude:pos.coords.latitude,longitude:pos.coords.longitude},{onConflict:'user_id'})
        fetchData()
      },
      async()=>{
  ...
  async function createLobby(e) {
    ...
    navigator.geolocation?.getCurrentPosition(
      async pos=>{
        await supabase.from('open_lobbies').insert({host_id:profile.id,...lobby,latitude:pos.coords.latitude,longitude:pos.coords.longitude,scheduled_at:new Date(lobby.scheduled_at).toISOString()})
  ...
  return (
    <div style={{maxWidth:'480px',margin:'0 auto',background:'#0A0000',minHeight:'100vh'}}>
      ...
      <div style={{background:'#0A0000',padding:'0 16px',height:'58px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:1000,borderBottom:'1px solid rgba(246,54,118,0.1)'}}>
        <Logo size="sm" />
        <button onClick={()=>nav('/dashboard')} style={{background:'rgba(255,255,255,0.05)',border:'1px solid rgba(255,255,255,0.08)',color:'#FFFFFF',padding:'6px 14px',borderRadius:'8px',fontSize:'12px',fontFamily:'Montserrat,sans-serif',fontWeight:700}}>← BACK</button>
      </div>
      ...
      {showSetup&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center',padding:'24px'}}>
          <div style={{background:'#140008',border:'1px solid rgba(246,54,118,0.3)',borderRadius:'20px',padding:'28px',width:'100%',maxWidth:'400px',maxHeight:'90vh',overflowY:'auto'}}>
            ...
            <div style={{marginBottom:'20px'}}>
              ...
              <textarea style={{...inp,minHeight:'60px',resize:'vertical'}} value={setup.bio} onChange={e=>setSetup(f=>({...f,bio:e.target.value}))} placeholder="e.g. Padel player, love doubles on weekends"/>
            </div>
          ...
          <div style={{background:'#140008',border:'1px solid rgba(246,54,118,0.3)',borderRadius:'20px',padding:'28px',width:'100%',maxWidth:'400px'}}>
            ...
            <form onSubmit={createLobby}>
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                ...
                <button type="submit" disabled={loading} style={{padding:'13px',borderRadius:'10px',border:'none',background:'#F63676',color:'#FFFFFF',fontSize:'12px',fontWeight:800,fontFamily:'Montserrat,sans-serif',letterSpacing:'0.5px'}}>{loading?'CREATING...':'OPEN LOBBY'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <div style={{padding:'16px',paddingBottom:'80px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'16px'}}>
          <button onClick={()=>setShowLobby(true)} style={{padding:'13px',borderRadius:'12px',border:'none',background:'#F63676',color:'#FFFFFF',fontSize:'12px',fontWeight:800,fontFamily:'Montserrat,sans-serif',letterSpacing:'0.5px',textTransform:'uppercase'}}>🏟 Open Lobby</button>
          <button onClick={()=>setShowSetup(true)} style={{padding:'13px',borderRadius:'12px',border:'1px solid rgba(246,54,118,0.3)',background:'transparent',color:'#F63676',fontSize:'12px',fontWeight:800,fontFamily:'Montserrat,sans-serif',letterSpacing:'0.5px',textTransform:'uppercase'}}>✏️ Edit Profile</button>
        </div>

        <div style={{display:'flex',gap:'8px',marginBottom:'16px'}}>
          {[['map','🗺️ Map'],['lobbies','🏟 Lobbies'],['players','👥 Players']].map(([id,label])=>(
        ...
        {tab==='lobbies'&&(
          openLobbies.length===0
            ?<div style={{textAlign:'center',color:'#757070',padding:'40px',fontFamily:'DM Sans,sans-serif'}}>No open lobbies right now. Be the first!</div>
            :openLobbies.map(l=>(
              <div key={l.id} style={{background:'rgba(255,255,255,0.03)',borderRadius:'14px',padding:'16px',border:'1px solid rgba(255,255,255,0.06)',marginBottom:'10px'}}>
                ...
                <button onClick={()=>requestJoin(l.id)} style={{width:'100%',padding:'11px',borderRadius:'8px',border:'none',background:'#F63676',color:'#FFFFFF',fontSize:'12px',fontWeight:800,fontFamily:'Montserrat,sans-serif',letterSpacing:'0.5px',textTransform:'uppercase'}}>REQUEST TO JOIN</button>
              </div>
            ))
        )}
